...
<script>
    ...
    @foreach($days as $day)
        @if(isset($scheduleData[$day]))
            @foreach($scheduleData[$day] as $index => $schedule)
                document.addEventListener('DOMContentLoaded', function() {
                    const startSelect = document.querySelector(`select[name="schedules[{{ $day }}][{{ $index }}][start_hour_id]"]`);
                    const endSelect = document.querySelector(`select[name="schedules[{{ $day }}][{{ $index }}][end_hour_id]"]`);
