    ...
    <form action="{{ route('manage-hours.update', $hour->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="session_type" class="form-control" required>
                <option value="Jam pelajaran" {{ $hour->session_type == 'Jam pelajaran' ? 'selected' : '' }}>Jam Pelajaran</option>
                <option value="Jam istirahat" {{ $hour->session_type == 'Jam istirahat' ? 'selected' : '' }}>Jam Istirahat</option>
            </select>
        </div>
        <div class="mb-3">
            <label for="slot_number">Jam ke-</label>
            <input type="number" name="slot_number" class="form-control" value="{{ $hour->slot_number }}" min="1" required>
        </div>
        <div class="mb-3">
